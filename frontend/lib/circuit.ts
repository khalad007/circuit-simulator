...
export function parseCircuit(value: unknown): { nodes: Node[]; edges: Edge[]; viewport?: Viewport } {
  ...
  const nodes: Node[] = value.nodes.map((n: unknown) => {
    if (!record(n) || typeof n.id !== 'string' || !n.id || ids.has(n.id) || typeof n.type !== 'string' || !componentTypes.includes(n.type) || !record(n.position) || !finite(n.position.x) || !finite(n.position.y) || Math.abs(n.position.x) > 1e6 || Math.abs(n.position.y) > 1e6 || (n.data !== undefined && !record(n.data))) throw new Error('Invalid or duplicate component, type, or position in circuit.');
    ids.set(n.id, n.type);
...
export async function api<T>(route: string, payload?: unknown, signal?: AbortSignal): Promise<T> {
  ...
  try {
    response = await fetch(`http://127.0.0.1:8000/api/${route}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload ?? {}), signal });
  } catch (error) {
    if (signal?.aborted) throw error;
    throw new Error('Cannot connect to the backend. Run uvicorn on port 8000 and try again.');
  }
