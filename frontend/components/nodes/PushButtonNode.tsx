...
export default function PushButtonNode({
  ...
  return (
    <div className="flex flex-col items-center border-2 border-gray-800 bg-red-50 rounded-md p-2 w-32 shadow-md relative">
      ...
      <button
        onMouseDown={() => data.onPushPress?.(id, true)}
        onMouseUp={() => data.onPushPress?.(id, false)}
        onTouchStart={() => data.onPushPress?.(id, true)}
        onTouchEnd={() => data.onPushPress?.(id, false)}
        className={`w-12 h-12 my-1 rounded-full border-4 font-bold text-xs flex items-center justify-center transition-transform shadow-inner ${
          isPressed
